...
H5PointwiseInterpCut[
  {HA_, HS_, h2r1_, h2r2_}, 
  interp_, 
  domain:{Repeated[{_, _}, {4}]}|"Extrapolated"|Automatic:"Extrapolated",
  def:_?NumericQ:10.^9, 
  min:_?NumericQ:0.
  ]:=
With[
...
$H5DipoleSurfaceVec:=
  $H5DipoleSurfaceVec=
    H5VectorizedInterpCut[{Automatic, Automatic, Automatic, Automatic}, #, 0]&/@
      $H5DipoleSurfaceInterps
...
loadH5DVR["Region"]:=
  (
    {h5RegMin,h5RegMax}=Transpose@$H5PotentialRegion[[{3, 4}]];
    h5RegUnrot=Rectangle[h5RegMin,h5RegMax];
    h5Reg=Region@TransformedRegion[h5RegUnrot,RotationTransform[\[Pi]/4]];
    )
...
loadH2SDVR["Region"]:=
  h2SingleReg=Region@Interval[{H5FullInterpPot[[1, 1, 1]], 1}];
